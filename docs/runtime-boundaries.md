...
from the shared Homeboy runner path.
...
- `.github/workflows/world-creator.yml` calls the Homeboy Extensions runner
  wrapper with World Creator domain inputs, the MDI preview drop-in, and the
  world bootstrap/probe hooks.
...
- World Creator PR capture uses the Homeboy runner workspace publication
  contract instead of direct Data Machine Code tool classes or tool-result
  projection wiring.
...
The Homeboy runner wrapper provides the runtime path for:

- WP Codebox provider selection and mount mechanics;
- Data Machine bundle execution ability names;
- runner workspace publication and PR-result plumbing;
...
The deployed Homeboy runner workflow still accepts hook files via
...
- World Creator PR capture uses the shared runner workspace publication
  contract instead of direct Data Machine Code tool classes.
